...
//Iterator: BEGIN
    ...
    private:
        Stack<T>* stack;
        typename DLinkedList<T>::BWDIterator listIt;
        //typename DLinkedList<T>::BWDIterator bIt;
    public:
        Iterator(Stack<T>* stack=0, bool begin=true){
            this->stack = stack;
            if(begin){
                if(stack != 0) this->listIt = stack->list.bbegin();
                else this->listIt = 0;
            }
            else{
                if(stack != 0) this->listIt = stack->list.bend();
                else this->listIt = 0;
            }
        }
